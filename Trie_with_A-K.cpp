...
constexpr char first_letter = '0';
constexpr size_t alphabet_size = 10;
...
private:
  ...
  void add_next_vertices(std::shared_ptr<Node> vertex,
...
void Trie::add_next_vertices(std::shared_ptr<Node> vertex,
    std::queue<std::shared_ptr<Node>>& output) {
...
std::vector<std::pair<std::string_view, size_t>> pattern_split(std::string_view pattern,
    ...
    if (pattern[i] == separator){
      if (i - last_subpattern_index > 0) {
        result.emplace_back(std::make_pair(
            std::string_view(pattern.substr(last_subpattern_index,
  ...
  if (last_subpattern_index != pattern.length()) {
    result.emplace_back(std::make_pair(
        pattern.substr(last_subpattern_index,pattern.length() -
        last_subpattern_index), pattern.length() - 1)
        );
  }
  return result;
}
